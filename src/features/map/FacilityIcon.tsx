...
const FacilityIcon = ({ category, className = '', showShadow = true }: FacilityIconProps) => {
  return (
    <span className={`flex h-12 w-12 shrink-0 items-center justify-center ${className}`}>
      <img
        ...
        className={`h-full w-full ${
          showShadow ? 'drop-shadow-[0_4px_6px_rgba(0,0,0,0.4)]' : 'drop-shadow-none'
        }`}
        style={showShadow ? undefined : { filter: 'none' }}
      />
    </span>
  );
};
